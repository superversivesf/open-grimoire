...
@pytest.mark.asyncio
async def test_runner_handles_dict_summary_from_llm(tmp_dirs):
    """LLM returning summary as a dict must not crash the enrich stage."""
    pdf_path = tmp_dirs["data"] / "test.pdf"
    _make_pdf(pdf_path)
    conn = init_shared_db(tmp_dirs["db"])
    uconn = init_user_db(tmp_dirs["db"], "alice")
    ...
    gw = MagicMock()
    gw.call = AsyncMock(return_value={"message": {"content": '{"summary": {"text": "Combat rules."}, "keywords": ["combat"]}'}})
    runner = PipelineRunner(gw, tmp_dirs["data"], tmp_dirs["db"])
    await runner.run_job(job)
